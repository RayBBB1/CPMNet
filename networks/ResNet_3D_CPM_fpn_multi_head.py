...
class FPN3D(nn.Module):
    def __init__(self, n_filters, feature_size=64, norm_type='batchnorm', act_type='ReLU'):
        ...
        self.p4_downsampled = DownsamplingConvBlock(feature_size, feature_size, norm_type=norm_type, act_type='none')
...
class Resnet18(nn.Module):
    def __init__(self, n_channels=1, n_blocks=[2, 3, 3, 3], n_filters=[64, 96, 128, 160], stem_filters=32,
        ...
        if out_stride == 4:
            ...
            self.head3 = ClsRegHead(in_channels=feature_size, feature_size=feature_size, conv_num=3, norm_type=head_norm, act_type=act_type)

        elif out_stride == 2:
            feature_size = n_filters[0]
            self.fpn = FPN3D(n_filters, feature_size=feature_size, norm_type=norm_type, act_type=act_type)
            # Head
            # self.heads = [ClsRegHead(in_channels=feature_size, feature_size=feature_size, conv_num=3, norm_type=head_norm, act_type=act_type) for _ in range(4)]
            self.head1 = ClsRegHead(in_channels=feature_size, feature_size=feature_size, conv_num=3, norm_type=head_norm, act_type=act_type)
            self.head2 = ClsRegHead(in_channels=feature_size, feature_size=feature_size, conv_num=3, norm_type=head_norm, act_type=act_type)
            self.head3 = ClsRegHead(in_channels=feature_size, feature_size=feature_size, conv_num=3, norm_type=head_norm, act_type=act_type)
            self.head4 = ClsRegHead(in_channels=feature_size, feature_size=feature_size, conv_num=3, norm_type=head_norm, act_type=act_type)
    ...
    def forward(self, inputs):
        ...
        Shapes.append(F.relu(Shape))
        Offsets.append(Offset)
        Clses.append(Cls)

        cls_feat2, shape_feat2, offset_feat2 = self.head2(feats[1])
        Shape = self.shape_output(shape_feat2)
        Offset = self.offset_output(offset_feat2)
        Cls = self.cls_output(cls_feat2)
        Shapes.append(F.relu(Shape))
        Offsets.append(Offset)
        Clses.append(Cls)

        cls_feat3, shape_feat3, offset_feat3 = self.head3(feats[2])
        Shape = self.shape_output(shape_feat3)
        Offset = self.offset_output(offset_feat3)
        Cls = self.cls_output(cls_feat3)
        Shapes.append(F.relu(Shape))
        Offsets.append(Offset)
        Clses.append(Cls)
...
class DetectionLoss(nn.Module):
    ...
    def fpn_format_annotation(annotations: torch.Tensor,
        ...
        for annots in annotations:
            ...
            for s in range(len(gt_bboxes)):
                ...
                annots_list[rank_index].append(each_label)
        ...
        if max_num_annots > 0:
            annot_padded = np.ones((len(annots), len(size_of_interst), max_num_annots, 10), dtype='float32') * -1
        for idx, annots in enumerate(annotations_list):
            for jdx, annot in enumerate(annotations_list[idx]):
                if annot.shape[0] > 0:
                    annot_padded[idx, jdx, :annot.shape[0], :] = annot
        else:
            annot_padded = np.ones((len(annots), len(size_of_interst), 1, 10), dtype='float32') * -1
        
        return torch.from_numpy(annot_padded, device=device)
    ...
    def forward(self, 
        ...
        # process annotations
        annotations_fpn = self.fpn_format_annotation(annotations, device, [0, 2, 4])
        print('annotations_fpn', annotations_fpn.shape)
