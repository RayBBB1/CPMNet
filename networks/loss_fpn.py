...
class DetectionLoss(nn.Module):
    ...
    @staticmethod
    def fpn_format_annotation(annotations: torch.Tensor,
                              device,
                              size_of_interst: List[int]) -> List[torch.Tensor]:
        '''
        Split the annotations into several sets, with each set including objects of different sizes following the `size_of_interest` 
        ...
        Returns: 
            annot_padded: torch.Tensor
                A tensor of shape (batch_size, num_object_size, num_annotations, 10) containing the annotations in the format:
                    (ctr_z, ctr_y, ctr_x, d, h, w, spacing_z, spacing_y, spacing_x, 0 or -1). The last index -1 means the annotation is ignored.
        '''
        ...
        for annots in annotations:
            annots_list = [[] for _ in range(len(size_of_interst))]
            ...
            for s in range(len(gt_bboxes)):
                ...
                max_side = max(d, h, w)
                rank_index = -1
                for range_index in range(len(size_of_interst)-1):
                    if max_side >= size_of_interst[range_index] and max_side < size_of_interst[range_index+1]:
                        rank_index = range_index
                if rank_index == -1:
                    rank_index = len(size_of_interst)-1
                annots_list[rank_index].append(each_label.numpy())
    
            temp_max_num_annots = max([len(annot) for annot in annots_list])
            if max_num_annots < temp_max_num_annots:
                max_num_annots = temp_max_num_annots
            annotations_list.append(annots_list)
        if max_num_annots > 0:
            annot_padded = np.ones((len(annotations_list), len(size_of_interst), max_num_annots, 10), dtype='float32') * -1
        for idx, annots in enumerate(annotations_list): # batch
            for jdx, annot in enumerate(annots): # size
                annot = np.array(annot)
                if annot.shape[0] > 0:
                    annot_padded[idx, jdx, :annot.shape[0], :] = annot
        else:
            annot_padded = np.ones((len(annotations_list), len(size_of_interst), max_num_annots, 10), dtype='float32') * -1
        return torch.tensor(annot_padded, device=device)
    @staticmethod
    def target_proprocess(annotations: torch.Tensor, 
        ...
        """
        size_of_interst = [0, 4, 8]
        batch_size = annotations.shape[0]
        # annotations_new = -1 * torch.ones_like(annotations, device=device)
        annotations_new = -1 * torch.ones(batch_size, len(size_of_interst), annotations.shape[1], annotations.shape[2], device=device)
        max_num_annots = 0
        for sample_i in range(batch_size):
            annots = annotations[sample_i]
            gt_bboxes = annots[annots[:, -1] > -1] # -1 means ignore, it is used to make each sample has same number of bbox (pad with -1)
            bbox_annotation_targets = [[] for _ in range(len(size_of_interst))]
            ...
            for s in range(len(gt_bboxes)):
                ...
                if (percent > 0.1) and (nw*nh*nd >= 15):
                    ...
                    bbox_annotation_targets[range_index].append(bbox.view(1, 10))
                else:
                    mask_ignore[rank_index][sample_i, 0, int(z1) : int(torch.ceil(z2)), int(y1) : int(torch.ceil(y2)), int(x1) : int(torch.ceil(x2))] = -1
            
            temp_max_num_annots = max([len(annot) for annot in bbox_annotation_targets])
            if max_num_annots < temp_max_num_annots:
                max_num_annots = temp_max_num_annots
            
            
            for size_index in range(len(size_of_interst)):
                if len(bbox_annotation_targets[size_index]) > 0:
                    bbox_annotation_target = torch.cat(bbox_annotation_targets[size_index], 0)
                    annotations_new[sample_i, size_index, :len(bbox_annotation_target), :] = bbox_annotation_target
        return annotations_new[:, :, :max_num_annots, :], mask_ignore
    ...
    def get_pos_target(annotations: torch.Tensor,
        ...
        target_bboxes = annotations[:, :, :6].clone().view(-1, 6)[gt_idx] # zyxdhw
    ...
    def forward(self, 
        ...
        Args:
            ...
            device: torch.device, the device of the model.
        """
        cls_pos_losses, cls_neg_losses, reg_losses, offset_losses, iou_losses = None, None, None, None, None
        strides = [4, 8, 16]
        Clses = output['Cls']
        Shapes = output['Shape']
        ...
        targets_mask_ignore = [torch.zeros(Cls.size()).to(device) for Cls in Clses]
        # process annotations
        process_annotations, targets_mask_ignore = self.target_proprocess(annotations, device, self.crop_size, targets_mask_ignore)
        for output_index in range(num_outputs):

            # view shape
            pred_scores = Clses[output_index].view(batch_size, 1, -1) # (b, 1, num_points)
            pred_shapes = Shapes[output_index].view(batch_size, 3, -1) # (b, 3, num_points)
            pred_offsets = Offsets[output_index].view(batch_size, 3, -1)
            # (b, num_points, 1 or 3)
            ...
            anchor_points, stride_tensor = make_anchors(Clses[output_index], self.crop_size, 0) # shape = (num_anchors, 3)
            ...
            target_offset, target_shape, target_bboxes, target_scores, mask_ignore = self.get_pos_target(annotations = process_annotations[:, output_index, :, :],
                                                                                                        anchor_points = anchor_points,
                                                                                                        stride = stride_tensor[0].view(1, 1, 3), 
                                                                                                        pos_target_topk = self.pos_target_topk,
                                                                                                        ignore_ratio = self.pos_ignore_ratio)
            # merge mask ignore
            target_mask_ignore = targets_mask_ignore[output_index].view(batch_size, 1,  -1)
            ...
            if output_index > 0:
                cls_pos_losses += cls_pos_loss
                cls_neg_losses += cls_neg_loss
                reg_losses += reg_loss
        ...
        return cls_pos_losses/num_outputs, cls_neg_losses/num_outputs, reg_losses/num_outputs, offset_losses/num_outputs, iou_losses/num_outputs