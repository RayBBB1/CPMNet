...
class DetectionLoss(nn.Module):
    ...
    @staticmethod
    def distribution_focal_loss(pred_regs, target_regs, mask_ignore, target, min_reg=0, max_reg=35, bins=36):
        mask = target.clone()
        mask[mask_ignore.long()] = 0
        mask = mask.squeeze(-1).bool()
        base = torch.linspace(min_reg, max_reg, bins, device=pred_regs.device, dtype=pred_regs.dtype)
        if torch.sum(mask) > 0:
            pred_shapes = pred_regs[mask].reshape(-1, bins)
            target_shapes = target_regs[mask].reshape(-1)
            disr = torch.searchsorted(base, target_shapes)
            disl = disr - 1

            wl = base[disr] - target_shapes
            wr = target_shapes - base[disl]
            loss = F.cross_entropy(pred_shapes, disl, reduction='none') * wl \
            + F.cross_entropy(pred_shapes, disr, reduction='none') * wr
                
            return torch.mean(loss)
        else:
            return torch.tensor(0.0, device=pred_regs.device)
    ...
    def forward(self, 
        ...
        Shape_feat = output['Shape_feat']
        Offset_feat = output['Offset_feat']
        ...
        shape_loss = self.distribution_focal_loss(pred_regs=Shape_feat, target_regs=target_shape, mask_ignore=mask_ignore, target=target_scores)
        offset_loss = self.distribution_focal_loss(pred_regs=Offset_feat, target_regs=target_offset, mask_ignore=mask_ignore, target=target_scores)
