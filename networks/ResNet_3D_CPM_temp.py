...
class CPM_net(nn.Module):
    def __init__(self, backbone=None, detection_loss=None, device=None, head_norm='batchnorm', act_type='ReLU'):
        super(CPM_net, self).__init__()
        self.detection_loss = detection_loss
        self.backbone = backbone
        
        self.device = device
        self.head = ClsRegHead(in_channels=self.backbone.out_channels, feature_size=self.backbone.out_channels, conv_num=3, norm_type=head_norm, act_type=act_type)
        self._init_weight()
    def forward(self, inputs):
        if self.training and self.detection_loss != None:
            x, labels = inputs
