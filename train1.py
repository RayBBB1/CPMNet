...
def get_args():
    ...
    # Model
    parser.add_argument('--model_class', type=str, default='networks.ResNet_3D_CPM_temp', help='model class')
    parser.add_argument('--backbone', type=str, default='networks.Reset18', help='model class')
...
def prepare_training(args, device, num_training_steps) -> Tuple[int, Any, AdamW, GradualWarmupScheduler, Any]:
    ...
    Cpm_net = build_class('{}.CPM_net'.format(args.model_class))
    Backbone = build_class('{}.Backbone'.format(args.backbone))
    ...
    backbone = Backbone(norm_type = args.norm_type,
    ...
    model = Cpm_net(head_norm = args.head_norm, 
                     act_type = args.act_type, 
